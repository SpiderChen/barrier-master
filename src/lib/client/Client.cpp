...
static const double kAudioPlaybackIdleStopSeconds = 3.0;
...
Client::handleHello(const Event&, void*)
{
    ...
    const SInt16 kMinSupportedMinorVersion = 6;
    if (major != kProtocolMajorVersion || minor < kMinSupportedMinorVersion) {
    ...
    // say hello back
    const SInt16 negotiatedMinorVersion =
        (minor < kProtocolMinorVersion) ? minor : kProtocolMinorVersion;
    m_serverProtocolMinor = negotiatedMinorVersion;
    LOG((CLOG_DEBUG1 "say hello version %d.%d", kProtocolMajorVersion, negotiatedMinorVersion));
    ProtocolUtil::writef(m_stream, kMsgHelloBack,
                            kProtocolMajorVersion,
                            negotiatedMinorVersion, &m_name);
...
void
Client::audioChunkReceived(UInt8 mark, const std::string& data)
{
    if (!m_args.m_enableAudio || m_audioPlayer == NULL) {
        return;
    }
    ...
    case kDataStart: {
        ...
        AudioFormat format;
        if (!AudioChunk::parseFormat(data, format)) {
            LOG((CLOG_ERR "invalid server audio format metadata"));
            return;
        }

        const bool hadPendingStop = m_audioStopTimer != NULL;
        ...
        if ((formatChanged || hadPendingStop || resumedAfterIdle) &&
    ...
    case kDataChunk: {
        const double now = ARCH->time();
        if (data.empty()) {
            if (m_audioPlayer->isRunning() && m_audioStopTimer == NULL) {
                scheduleAudioPlaybackStop(kAudioPlaybackIdleStopSeconds, false);
            }
            break;
        }
        ...
        cancelAudioPlaybackStop();
        if (!hadPendingStop &&
            m_audioPlayer->isRunning() && m_hasAudioFormat &&
            m_lastAudioChunkTime > 0.0 &&
            now - m_lastAudioChunkTime >= kAudioPlaybackReopenIdleSeconds) {
            LOG((CLOG_NOTE "client audio playback resumed after idle; reopening device"));
    ...
    if (isReceivedFileSizeValid()) {
        if (!m_clipboardFileList.empty()) {
            String path = DropHelper::writeToFileClipboardCache(
                                                 ...
                                                 m_receivedFileData);
            if (!path.empty()) {
                m_receivedClipboardFilePaths.push_back(path);
            }
            if (m_clipboardFileList.empty()) {
                setLocalFileClipboard();
            }
        }
        else {
...
void
Client::fileClipboardReceived(UInt32 fileNum, std::string data)
{
    if (!m_args.m_enableDragDrop) {
        LOG((CLOG_DEBUG "file copy/paste not enabled, ignoring copied file list."));
        return;
    }

    m_receivedClipboardFilePaths.clear();
    DragInformation::parseDragInfo(m_clipboardFileList, fileNum, data);
}
...
void
Client::sendFilesToServer(const FilePathList& filenames)
{
    if (m_sendFileThread != NULL) {
        StreamChunker::interruptFile();
    }
...
void
Client::send_files_thread(FilePathList filenames)
{
    try {
        ...
        LOG((CLOG_ERR "failed sending copied file chunks: %s", error.what()));
    }
...
void
Client::sendClipboardFiles(const IClipboard* clipboard)
{
    if (!m_args.m_enableDragDrop || m_serverProtocolMinor < 7 || clipboard == NULL) {
        return;
    }
    if (!clipboard->open(0)) {
        return;
    ...
    if (clipboard->has(IClipboard::kFiles)) {
        fileData = clipboard->get(IClipboard::kFiles);
    }
    clipboard->close();
    ...
    String transferInfo;
    UInt32 fileCount = FileClipboard::setupTransferInfo(
        FileClipboard::split(fileData), transferablePaths, transferInfo);
    if (fileCount == 0) {
        return;
    }

    m_server->sendFileClipboard(fileCount, transferInfo.c_str(), transferInfo.size());
    sendFilesToServer(transferablePaths);
}

void
Client::setLocalFileClipboard()
{
    if (m_receivedClipboardFilePaths.empty()) {
        return;
    }
    ...
    if (clipboard.open(0)) {
        clipboard.empty();
        clipboard.add(IClipboard::kFiles, FileClipboard::join(m_receivedClipboardFilePaths));
        clipboard.close();
