...
ServerProxy::parseMessage(const UInt8* code)
{
    ...
    else if (memcmp(code, kMsgDAudioTransfer, 4) == 0) {
        audioChunkReceived();
    }
    else if (memcmp(code, kMsgDFileClipboard, 4) == 0) {
...
void
ServerProxy::audioChunkReceived()
{
    UInt8 mark = 0;
    std::string content;
    if (!AudioChunk::read(m_stream, mark, content)) {
        return;
    }

    m_client->audioChunkReceived(mark, content);
}

void
ServerProxy::fileClipboardReceived()
{
    UInt32 fileNum = 0;
    std::string content;
    ProtocolUtil::readf(m_stream, kMsgDFileClipboard + 4, &fileNum, &content);

    m_client->fileClipboardReceived(fileNum, content);
}
...
void
ServerProxy::sendFileClipboard(UInt32 fileCount, const char* info, size_t size)
{
    std::string data(info, size);
    ProtocolUtil::writef(m_stream, kMsgDFileClipboard, fileCount, &data);
}
