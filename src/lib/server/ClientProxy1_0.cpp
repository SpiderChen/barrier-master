...
void
ClientProxy1_0::audioChunkSending(UInt8 mark, const char* data, size_t dataSize)
{
    // ignore -- not supported in protocol 1.0
    LOG((CLOG_DEBUG "audioChunkSending not supported"));
}

void
ClientProxy1_0::sendFileClipboard(UInt32 fileCount, const char* info, size_t size)
{
    // ignore -- not supported in protocol 1.0
    LOG((CLOG_DEBUG "fileClipboardSending not supported"));
}
