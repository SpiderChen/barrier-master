	...
	m_args(args)
{
  ...
  m_events->adoptHandler(m_events->forServer().toggleScreen(),
	...
	if (m_args.m_enableAudio) {
		m_events->adoptHandler(m_events->forAudio().audioChunkSending(),
								this,
								new TMethodEventJob<Server>(this,
									&Server::handleAudioChunkSendingEvent));
	}
		...
		if (m_enableClipboard) {
			...
			for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
				...
				if (id == kClipboardClipboard) {
					sendClipboardFilesToActive(&m_clipboards[id].m_clipboard);
				}
			}
		}
...
void
Server::handleAudioChunkSendingEvent(const Event& event, void*)
{
	onAudioChunkSending(event);
}
				...
				ClipboardID id, UInt32 seqNum)
{
	...
	if (id == kClipboardClipboard) {
		sendClipboardFilesToActive(&clipboard.m_clipboard);
	}
}
...
void
Server::onAudioChunkSending(const Event& event)
{
...
Server::send_files_thread(FilePathList filenames)
{
	try {
		...
		LOG((CLOG_ERR "failed sending copied file chunks, error: %s", error.what()));
	}
...
void
Server::audio_chunk_callback(AudioChunk* chunk, void* context)
{
...
void
Server::fileClipboardReceived(UInt32 fileNum, std::string content)
{
	if (!m_args.m_enableDragDrop) {
	...
	DragInformation::parseDragInfo(m_clipboardFileList, fileNum, content);
}

void
Server::sendClipboardFilesToActive(const IClipboard* clipboard)
{
	if (!m_args.m_enableDragDrop || m_active == NULL || m_active == m_primaryClient || clipboard == NULL) {
		return;
	}
	if (!clipboard->open(0)) {
		return;
