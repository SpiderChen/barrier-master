...
// 1.7:  adds server audio streaming
...
// audio data: primary -> secondary
// transfer server audio. A mark is used in the first byte.
// 0 means the content followed is audio stream format metadata.
// 1 means the content followed is PCM audio chunk data.
// 2 means the audio stream is finished.
extern const char*        kMsgDAudioTransfer;

// file clipboard information: primary <-> secondary
// transfer copied file list metadata before the file chunks are sent.
// $1 = number of files, $2 = comma-separated path and size pairs.
extern const char*        kMsgDFileClipboard;
